...
export default function Card({ joke }: JokeProps) {
  ...
  return (
    <div className='flex gap-10 items-center justify-center'>
      <motion.div
        className={`w-[300px] shadow-2xl h-[412px] rounded-lg flex items-center justify-center transition-colors duration-600 ${flippedBlueCard ? 'bg-[#370A77]' : ''}`} 
        initial={{ rotateY: 0 }}
      ...
      <motion.div
        className={`w-[300px] shadow-2xl h-[412px] rounded-lg flex items-center justify-center transition-colors duration-600 ${flippedRedCard ? 'bg-[#F55F56]' : ''}`} 
        initial={{ rotateY: 0 }}
